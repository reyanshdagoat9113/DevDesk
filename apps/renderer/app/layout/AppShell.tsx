        ...
        <div className={cn('p-2', showLabels ? 'px-2' : 'px-1.5')}>
          <button
            ...
            onClick={() => {
              ...
              window.dispatchEvent(new KeyboardEvent('keydown', { key: 'k', ...(isMac ? { metaKey: true } : { ctrlKey: true }) }))
            }}
          >
      ...
      <div className="relative flex flex-1 flex-col overflow-hidden">
        <header className="z-10 flex h-12 shrink-0 items-center justify-between border-b border-border bg-card px-4 shadow-sm">
          <div className="flex items-center gap-3">
            <Button type="button" variant="ghost" size="icon" aria-label={isSidebarExpanded ? 'Collapse sidebar' : 'Expand sidebar'} aria-expanded={isSidebarExpanded} aria-controls="devdesk-sidebar" onClick={() => setIsSidebarExpanded((expanded) => !expanded)} className="shrink-0 text-muted-foreground hover:text-foreground">
