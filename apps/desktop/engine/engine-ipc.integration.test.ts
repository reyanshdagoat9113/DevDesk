...
beforeEach(() => {
  ...
  cpSync(path.join(process.cwd(), 'apps', 'desktop', 'engine', 'fixtures', 'engine-sample'), projectRoot, {
    recursive: true,
  })
})
...
test('Electron IPC exposes the engine integration end-to-end', async () => {
  ...
  const startedEvents: Array<{ projectId: string }> = []
  const completedEvents: Array<{ projectId: string; result: { ok: boolean; filesIndexed: number } }> = []

  const stopStarted = electronAPI.onEngineIndexingStarted((payload: { projectId: string }) => {
    startedEvents.push(payload)
  })
  const stopCompleted = electronAPI.onEngineIndexingCompleted(
    (payload: { projectId: string; result: { ok: boolean; filesIndexed: number } }) => {
  ...
  assert.deepEqual(initialState.searchSessions, {})
  ...
  assert.equal(startedEvents[0].projectId, projectId)
  assert.equal(completedEvents.length, 1)
  assert.equal(completedEvents[0].projectId, projectId)

  const indexedState = await electronAPI.getEngineState()
  assert.ok(indexedState.indexes[projectId])
  assert.equal(indexedState.indexes[projectId].fileCount, 2)
  assert.equal(indexedState.indexes[projectId].dbPath, path.join(userDataRoot, 'engine', `${projectId}.sqlite`))

  const searchResult = await electronAPI.searchProjectContent(projectId, 'needle', { regex: true, limit: 10 })
  assert.equal(searchResult.ok, true)
  assert.equal(searchResult.query, 'needle')
  assert.ok(searchResult.results.length >= 1)
  assert.equal(searchResult.results[0].path, 'src/app.ts')
  assert.ok(searchResult.results[0].matches.length >= 1)

  const afterSearchState = await electronAPI.getEngineState()
  assert.ok(afterSearchState.searchSessions[projectId])
  assert.equal(afterSearchState.searchSessions[projectId].query, 'needle')
  assert.equal(afterSearchState.searchSessions[projectId].regex, true)

  const statsResult = await electronAPI.getProjectStats(projectId)
  assert.equal(statsResult.ok, true)
  assert.equal(statsResult.db, path.join(userDataRoot, 'engine', `${projectId}.sqlite`))
  assert.equal(statsResult.stats.totalFiles, 2)

  const clearSearchResult = await electronAPI.clearProjectSearchSession(projectId)
  assert.deepEqual(clearSearchResult, { success: true })

  const clearIndexResult = await electronAPI.clearProjectIndex(projectId)
  assert.deepEqual(clearIndexResult, { success: true })

  const clearedState = await electronAPI.getEngineState()
  assert.deepEqual(clearedState.indexes, {})
  assert.deepEqual(clearedState.searchSessions, {})
